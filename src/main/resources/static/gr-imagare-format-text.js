...
function _getPreferences() {
  return imagareRestApi.get('')
    .then(prefs => {
      ...
      plugin.fire('show-error', { message: response });
    });
}
...
Gerrit.install(plugin => {
  ...
  imagareRestApi = plugin.restApi('/accounts/self/imagare~preference');

  let formatApi = plugin.formatTextApi();
  formatApi.addFormatTextCallback(decorateLinks);
