...
(function () {
  ...
  Polymer({
    ...
    _computeUploadAllDisabled() {
      return document.getElementById('imageList').getC
    },
    ...
    _createImageObject(name, data, url, list_entry) {
      return {
        data: data,
        name: name,
        url: url,
    ...
    _uploadImage(image) {
      if (image.list_entry && image.list_entry.uploaded) {
        return;
      }
      ...
      this.plugin.restApi('/projects')
        ...
        .then(response => {
          ...
          this._images.set(
            image.name,
            this._createImageObject(image.name, image.data, response.url, image.list_entry));
